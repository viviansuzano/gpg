...
public:
	...
	bool initMarkers(int num) {
		...
		if (num == 0){
			rgba = {0.14, 0.83, 0.3, 1.0};
		} else if (num == 1){
			rgba = {0.13, 0.16, 0.85, 0.78};
		} else{
			rgba = {0.45, 0.45, 0.5, 1.0};
		}
	
		visualization_msgs::Marker pointMarker = getSphereMarker("world","foot_pos_FL", id[0], 0.05, rgba[0], rgba[1], rgba[2], rgba[3]);
		footPosFL_.second = pointMarker;

		pointMarker = getSphereMarker("world","foot_pos_FR", id[1], 0.05, rgba[0], rgba[1], rgba[2], rgba[3]);
		footPosFR_.second = pointMarker;

		pointMarker = getSphereMarker("world","foot_pos_RL", id[2], 0.05, rgba[0], rgba[1], rgba[2], rgba[3]);
		footPosRL_.second = pointMarker;

		pointMarker = getSphereMarker("world","foot_pos_RR", id[3], 0.05, rgba[0], rgba[1], rgba[2], rgba[3]);
